package org.example.codingtest.level3.숫자_야구
...
//    fun solution(number: Int) = number.toString().toList()
    ...
    // 1. 기본형 랜덤
    fun getRandomNumber() = Random.Default.nextInt(1000, 9999)


    fun createList(): List<Int> {
        val first = Random.Default.nextInt(1, 10)   // 첫 자리는 1~9
    ...
    fun changeList(list : List<Int>): List<Int>{
        val first = list.first().takeIf { it == -1 } ?: Random.Default.nextInt(1,10)
